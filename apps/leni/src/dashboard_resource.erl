...
-module(dashboard_resource).

%% --------------------------------------------------------------------
%% External exports
%% --------------------------------------------------------------------
-export([init/1, to_html/2, to_stream/2, content_types_provided/2, allowed_methods/2, resource_exists/2]).
...
content_types_provided(ReqData, Context) ->
    {[{"text/html", to_html},{"text/event-stream", to_stream}],ReqData, Context}.
%
...
stream() ->    
    receive
        ...
        %% stream.
        {gen_event_EXIT, thing_event, _} -> {<<>>, done};
        %% We got a cell update, send it to the stream and recurse.
        {Thing, Date, Value} -> Body = io_lib:format("data: ~s~ndata: ~s~ndata: ~s~n~n",[Thing, encode(date:get_formated_date_for_now(Date)), encode(Value)]),                        
                                {Body, fun stream/0}
    end.
